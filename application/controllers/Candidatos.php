...
class Candidatos extends CI_Controller
{
    ...
    { //atualização dos dados pessoais dos candidatos já cadastrados
    ...
    { //página de cadastro do candidato na área pública
        $pagina['menu1']='Candidatos';
...
<br><a href=\"".base_url()."\">Voltar</a></strong>
    ...
    public function ListaCandidatos($inativo = 0, $paginacao = 1)
    {
        if (!$this -> session -> logado) {
            redirect('Publico');
        } elseif ($this -> session -> perfil != 'sugesp' && $this -> session -> perfil != 'orgaos' && $this -> session -> perfil != 'administrador') {
    ...
    { //função de preenchimento da combo da view de cadastro
        $this -> load -> model('Candidatos_model');
    ...
    public function valida_genero_optativo()
    { //callback de validação customizada do formulário de cadsatro
    ...
    public function valida_cidade_estrangeira()
    { //callback de validação customizada do formulário de cadsatro
